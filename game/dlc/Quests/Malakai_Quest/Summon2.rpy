init python:
    @RegisterBattleSkill("Summon2")
    class BattleSkill_Summon2(BattleSkill):
        ...
        SummonedCharID = "e_summoned_voidterror"
        SummonStatMultiplier = {1: 1.00, 2: 1.25, 3: 1.50, 4: 1.75}

        def CanExecute(self):
            if not super(BattleSkill_Summon2, self).CanExecute():
                return False
        ...
        def GetDesc(self, DescLevel = 1):
            ...
            return tra(_("Summons a Void Spirit into an empty allied slot. Its health, damage, energy, and attributes are increased by %s%%. The guardian acts from the next turn. Only one guardian may be summoned at a time.")) % StatBonus
