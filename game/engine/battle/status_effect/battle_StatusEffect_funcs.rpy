            ...
            for BattleChar in AllSideChars:
                for SlotID in EQP_SLOTS.ALL:
                    if BattleChar.CharRef[SlotID] is not None:
                        ...
                        Assert(ItemID in store.all_items, "wtf? item id %s not found in all_items" % ItemID)
                        ItemDef = store.all_items[ItemID]
                        if "battle_perma_effects" in ItemDef:
                            if ItemDef["battle_perma_effects"] is not None:
                                ...
                                for EffID in EffIDs:
                                    ...
                                    elif EffID == "star_bringers_stealmanastrike":
                                        Battle_ApplyStatusEffect(
                                            TargetChar = BattleChar,
                                            StatusEffect = BattleStatusEff_StarBringerManaStealPermanent(
                                                SourceName = tra(_("Star Bringer")),
                                            )
                                        )
                                    elif EffID == "shadowreach_stealhpstrike":
                                        Battle_ApplyStatusEffect(
                                            TargetChar = BattleChar,
                                            StatusEffect = BattleStatusEff_ShadowreachHPStealPermanent(
                                                SourceName = tra(_("Shadowreach")),
                                            )
                                        )
