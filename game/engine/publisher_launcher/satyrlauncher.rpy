...
init python:
    ...
    # Safe fallbacks to prevent "given a default a second time" exceptions across the entire store
    _defaults = {
        "has_internet": None,
        "launcher_cover_items": [],
        "launcher_feed_loading": False,
        "launcher_feed_error": None,
        "launcher_thumb_xpos": -config.screen_width * 7 // 8 - 50,
        "launcher_thumb_txpos": -config.screen_width * 7 // 8 - 50,
        "launcher_downloads_requested": False,
        "launcher_show": False,
        "launcher_show_bg": True,
        "launcher_cover_selected": 1,
        "launcher_steam_build": True,
    }
    for var_name, default_val in _defaults.items():
        if not hasattr(store, var_name):
            setattr(store, var_name, default_val)

    if not hasattr(persistent, "launcheroptimization"):
        persistent.launcheroptimization = False
