# TODO: Translation updated at 2026-01-19 18:56

# game/data/characters/luciusmal/dialogue.rpy:49
translate ja luciusmal_talk_42c75cc3:

    # LUCIUSMAL "Hello traveler! Need anything?"
    LUCIUSMAL "やあ、旅人さん!何か必要なものは？"

# game/data/characters/luciusmal/dialogue.rpy:55
translate ja luciusmal_shop_4a23ec27:

    # LUCIUSMAL "I don't have much at the moment I'm afraid, let me see what I can rustle up for you."
    LUCIUSMAL "申し訳ありません、今はあまり手持ちがなくて…。何とかご用意できるか見てみますね。"

# game/data/characters/luciusmal/dialogue.rpy:64
translate ja luciusmal_shopOpen_c1ac3ef3:

    # LUCIUSMAL "Take a look."
    LUCIUSMAL "これを見て。"

# game/data/characters/luciusmal/dialogue.rpy:69
translate ja luciusmal_bye_fa6efd9c:

    # LUCIUSMAL "Come back anytime, friend!"
    LUCIUSMAL "いつでもおいで、友よ!"

translate ja strings:

    # game/data/characters/luciusmal/dialogue.rpy:18
    old "Let me see your stock."
    new "在庫を見せてください。"






