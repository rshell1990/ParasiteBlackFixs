# TODO: Translation updated at 2026-07-01 11:57

translate ptBR strings:

    # game/data/characters/lukkan/char.rpy:1
    old "Lukkan"
    new "Lukkan"

    # game/data/characters/lukkan/char.rpy:11
    old "A high-ranking officer who seems to handle various bureaucratic jobs. He sided with Alcott during the rebellion."
    new "Um oficial de alta patente que parece lidar com vários trabalhos burocráticos. Ele ficou do lado de Alcott durante a rebelião."

